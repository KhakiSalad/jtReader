...
from jt_reader.codec.deeringNormal import DeeringNormalCodec
...
class CompressedVertexNormalArray:
    ...
    def from_bytes(cls, e_bytes):
        normal_count, number_components, quantization_bits = struct.unpack("<iBB", e_bytes.read(6))
        if quantization_bits == 0:
            ...
            for i in range(number_components):
                ...
                normal_code = [(e << 23 | m) & 0xffffffff for e, m in zip(normal_exponents, normal_mantissae)]
            ...
            vertex_normal_coordinates = [[p[0] for p in xyz], [p[1] for p in xyz], [p[2] for p in xyz]]
